 ...
 * @param format Format type: 'full' | 'short' | 'digital'
 * @returns Formatted duration string
 *
 * Examples:
 * fDuration(3661, 'full') => "1 hour, 1 minute, 1 second"
 * fDuration(3661, 'short') => "1h 1m 1s"
 * fDuration(3661, 'digital') => "01:01:01"
 */
export function fDuration(
  seconds: number,
  format: 'full' | 'short' | 'digital' = 'digital'
): string {
  ...
  switch (format) {
    ...
    case 'short': {
      ...
      if (secs > 0 || shortParts.length === 0) shortParts.push(`${secs}s`);
 ...
 */
export function fDurationMinSec(seconds: number): string {
