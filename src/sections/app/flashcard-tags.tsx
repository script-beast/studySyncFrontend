...
export function FlashcardTags({
  title,
  subheader,
  totalCat = 0,
  avgFlashcardPerCat = 0,
  chart,
  sx,
  ...other
}: Props) {
    ...
    <Card sx={sx} {...other}>
      ...
      {chart.series.length === 0 ? (
        ...
        >
          ...
          <ChartLegends
            ...
            icons={chart.icons}
            // sublabels={chart.series.map((item) => item.value.toFixed(2))}
      ...
      >
        <Box sx={{ py: 2, borderRight: `dashed 1px ${theme.vars.palette.divider}` }}>
          <Box sx={{ mb: 1, typography: 'body2', color: 'text.secondary' }}>Total Categories</Box>{' '}
          {totalCat}
        </Box>

        <Box sx={{ py: 2 }}>
          <Box sx={{ mb: 1, typography: 'body2', color: 'text.secondary' }}>Avg. Flashcards</Box>
          {avgFlashcardPerCat}
        </Box>
      </Box>
    </Card>
