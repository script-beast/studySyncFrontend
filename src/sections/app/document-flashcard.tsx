...
export function DocumentFlashcard({ title, subheader, chart, sx, ...other }: Props) {
  const theme = useTheme();

  const total = sumBy(chart.series, (series) => series.value);

  const chartSeries =
    (chart.series.filter((i) => i.label === 'Flashcard Generated')[0].value / total) * 100;
    ...
    plotOptions: {
      radialBar: {
        ...
        dataLabels: {
          name: { offsetY: -12 },
          value: { offsetY: 6 },
          total: { label: 'Documents', formatter: () => total.toFixed(2) },
        },
      },
    },
    ...chart.options,
  ...
  return (
    <Card sx={sx} {...other}>
      ...
      <Chart
        ...
        series={[chartSeries]}
        options={chartOptions}
        sx={{ mx: 'auto', width: 210, height: 210 }}
      />
          ...
          >
            <Box
              sx={{
                ...
                ...(item.label === 'Flashcard Generated' && { bgcolor: chartColors[1] }),
              }}
            />
